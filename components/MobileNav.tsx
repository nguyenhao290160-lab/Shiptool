      ...
      <div className="max-w-3xl mx-auto flex justify-between items-center h-16 px-1">
        {items.map((it) => {
          ...
          return (
            ...
            >
              {active && (
                <span className="absolute -top-px left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-full bg-gradient-to-r from-cyan-500 to-cyan-600" />
              )}
              <div className={`p-1.5 rounded-xl transition-colors ${active ? "bg-cyan-50 text-cyan-600" : "bg-transparent"}`}>
                {it.icon}
              </div>
