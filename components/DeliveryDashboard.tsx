...
export const DeliveryDashboard = ({ orders }: Props) => {
  ...
  const pending = orders.filter((o) => o.status === "pending" || o.status === "ready" || o.status === "assigned").length;
  const delivering = orders.filter((o) => o.status === "delivering").length;
  const delivered = orders.filter((o) => o.status === "delivered").length;
  const missingCoords = orders.filter((o) => !o.lat || !o.lng).length;
  ...
  const stats: StatItem[] = [
    ...
    { label: "Thiếu tọa độ", value: missingCoords, dot: "bg-rose-400", bg: "bg-rose-50 border-rose-200" },
  ];
